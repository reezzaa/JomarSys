...
class EquipTypeController extends Controller
{
    public function index()
    {
        ...
        return view('layouts.mainte.equiptype.index', compact('equiptypeCheck'));
    }
    ...
    public function readByAjax()
    {
    	...
    	return view('layouts.mainte.equiptype.table', compact('equiptype'));
    }

    public function store(Request $request)
    {
        $equipdescAdd = EquipType::where('strEquipTypeDesc', '=', $request->strEquipTypeDesc )
            ->where('todelete','=',1)
            ->get();
        if($equipdescAdd->count() == 0)
        {
            EquipType::insert(['strEquipTypeDesc'=>$request->strEquipTypeDesc,
                'todelete'=>1,
                'status'=>1
                ]);
    ...
    {
        $equipdesc = EquipType::where('strEquipTypeDesc', '=', $request->strEquipTypeDesc )
                ->where('todelete','=',1)
                ->get();
        if($equipdesc->count() == 0)
        {
            $updclass = EquipType::find($typeID);
            $updclass->strEquipTypeDesc = $request->strEquipTypeDesc;
            $updclass->save();
