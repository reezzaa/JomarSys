<table id="worker-datatable" class="table table-vcenter table-striped table-condensed table-bordered table-hover">
  ...
  <tbody id="emp-list">
    ...
    <tr id="id{{$u->strEmpID}}">
        ...
        <td class="text-center">
            {{ $u->strEmpFirstName }} {{ $u->strEmpMiddleName }} {{ $u->strEmpLastName }}
        </td>
        <td class="text-center">
            {{ $u->strEmpProvince }}
        </td>
          ...
          <label class="switch switch-primary">
            @if(($u->status)==1)
              ...
              @else
                  <p hidden>0</p> 
              @endif
            <input name="status" id="status" type="checkbox" value="{{ $u->strEmpID }}" 
              @if(($u->status)==1)
                  {{"checked"}}
              @else
                  {{""}} 
        ...
        <td>
          <div class="text-center">
            <button id="view_supp" class="btn btn-alt btn-info view_supp" value = "{{$u->strEmpID}}" data-toggle="tooltip" data-placement="top" data-original-title="View"><span class="gi gi-eye_open"></span>
            </button>
            <button id="edit_supp" class="btn btn-alt btn-warning edit_supp" value = "{{$u->strEmpID}}" data-toggle="tooltip" data-placement="top" data-original-title="Edit"><span class="gi gi-pencil"></span>
            </button>
            <button id="del_supp" class="btn btn-alt btn-danger del_supp" value="{{$u->strEmpID}}" data-toggle="tooltip" data-placement="top" data-original-title="Delete"> <span class="gi gi-bin"></span>
            </button>
          </div>
      </td>
    </tr>
        ...
        <div class="block">
          <div class="block-title themed-background">
            ...
            {!! Form::open(['url'=>'worker', 'method'=>'PATCH', 'id'=>'form-validation', 'class' => 'form-horizontal form-bordered']) !!}
              @include('layouts.mainte.employee.formedit')
            {!! Form::close() !!}
        </div>
      </div>
    </div>
