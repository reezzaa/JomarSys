   ...
   <tbody id="delt-list">
    ...
    <tr>
        <td class="text-center">
            {{ $u->strDeliTruckPlateNo }}
        </td>
        <td class="text-center">
            {{ $u->strDeliTruckVINNo }}
        </td>
        <td class="text-center">
            {{ $u->dblDeliTruckCapacity }}
        </td>
        <td class="text-center">
            {{ $u->dblDeliTruckGrossWeight }}
        </td>
        <td class="text-center">
          <label class="switch switch-primary">
            @if(($u->status)==1)
              ...
              @else
                  <p hidden>0</p> 
              @endif
            <input name="status" id="status" type="checkbox" value="{{ $u->intDeliTruckID }}" 
              @if(($u->status)==1)
                  {{"checked"}}
              @else
                  {{""}} 
        ...
        <td>
          <div class="text-center">
            <button id="edit_supp" class="btn btn-alt btn-warning edit_supp" value = "{{$u->intDeliTruckID}}" data-toggle="tooltip" data-placement="top" data-original-title="Edit"><span class="gi gi-pencil"></span>
            </button>
            <button id="del_supp" class="btn btn-alt btn-danger del_supp" value="{{$u->intDeliTruckID}}" data-toggle="tooltip" data-placement="top" data-original-title="Delete"> <span class="gi gi-bin"></span>
            </button>
          </div>
